...
export const LANGUAGE_PROFILES: Record<string, LanguageProfile> = {
  ko: { code: "ko", isLatinScript: false, systemPromptRules: ["Use 존댓말 (-요/-습니다) for workplace, interview, or formal scenes. Use 반말 only when speakers are clearly close friends or family.", "Sentence-initial words functioning as softeners or attention-getters in professional contexts are not romantic address terms. Translate by pragmatic function or omit — never use 자기야/여보 unless a romantic relationship is confirmed in the same segment."], cleanupTranslation: (text) => text.replace(STRIP_SINGLE_PERIOD, "").trim(), isValidTranslation: (text) => /[가-힣]/.test(text), trailingPunctuationToStrip: STRIP_SINGLE_PERIOD },
